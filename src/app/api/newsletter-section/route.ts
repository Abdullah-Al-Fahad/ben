...
export async function GET() {
  try {
    ...
    return NextResponse.json(db.settings.newsletterSection);
  } catch (error) {
...
export async function PUT(req: NextRequest) {
  try {
    const updatedNewsletterSection = await req.json();
    const dbRaw = await fs.readFile(DB_PATH, 'utf-8');
    const db = JSON.parse(dbRaw);

    db.settings.newsletterSection = updatedNewsletterSection;
